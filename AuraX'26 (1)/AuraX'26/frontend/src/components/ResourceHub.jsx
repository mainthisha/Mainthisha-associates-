        ...
        {filtered.map(res => (
          <div key={res.id} className="glass-card" style={{ padding: 20 }}>
            <div style={{ marginBottom: 10 }}>
              <div style={{
                fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'var(--neon)',
            ...
            <a href={res.link} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'var(--neon)', textDecoration: 'none', borderBottom: '1px solid rgba(0,255,106,0.3)', paddingBottom: 2 }}>
              Access Resource
            </a>
          </div>
        ))}
              ...
              style={{
                ...
                border: `1px solid ${
                  requestSent[mentor.id]
                    ? 'rgba(0,255,106,0.3)'
                    : mentor.available ? 'var(--border-bright)' : 'var(--border)'
                }`,
                background: requestSent[mentor.id] ? 'rgba(0,255,106,0.08)' : 'transparent',
                color: requestSent[mentor.id]
                  ? 'var(--neon)'
                  : mentor.available ? 'var(--text-secondary)' : 'var(--text-muted)',
                borderRadius: 4,
                cursor: mentor.available && !requestSent[mentor.id] ? 'pointer' : 'not-allowed',
        ...
        <div className="modal-overlay" onClick={() => setShowForm(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div style={{
              ...
              color: 'var(--neon)', letterSpacing: '0.05em', marginBottom: 24,
            }}>
              Request Mentor Help
            </div>
      ...
      <div style={{ maxWidth: 600, marginBottom: 32 }}>
        <div style={{ position: 'relative' }}>
          <span style={{
            ...
            transform: 'translateY(-50%)', color: 'var(--neon)', fontSize: '1rem',
          }}>
            🔍
          </span>
          <input
            ...
            placeholder="Search: venue, registration fee, food..."
        ...
        {results.map(faq => (
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{
                color: 'var(--neon)', fontFamily: 'var(--font-display)',
                fontSize: '0.8rem', marginLeft: 12,
              }}>
                {expanded === faq.id ? '−' : '+'}
              </span>
            </div>
